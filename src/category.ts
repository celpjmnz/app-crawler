...
import { Product, FinalProduct, Category } from "./interfaces";

export const category = async (catId: string) => {
  const res = await getCategoryProducts(catId);
...
export const categories = async () => {
  ...
  if (res.status !== 200) throw new Error("Unable to fetch");
  const data: Category[] = res.data.data.categories;
  ...
  return {
    total_number_categories: categories.length,
    categories,
  };
};
